...
| Merge policy | Merge a pull request as soon as CI passes and an independent reviewer has no further feedback. If it changes the factory, the `Factory Approval` check requires either a trusted GitHub review of the current head or a verified current-head commit signed by the repository owner. Publishing and account changes still require explicit authority |
