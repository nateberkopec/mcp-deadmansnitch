...
- Action: investigate invariant violations and their causes; identify practical ways to move invariants to Enforced; deduplicate findings; rank validated improvements by expected impact and effort; update `docs/automations/factory-improvement/backlog.md`; and open a PR. Because it changes the factory, require current-head human approval in addition to passing CI and an independent reviewer with no further feedback. Change an invariant's status only in the same change that adds passing enforcement proof.
